...
class ListShiftsByDateQueryHandler
{
    ...
    public function __invoke(ListShiftsByDateQuery $query): ListShiftsByDateResponse
    {
        $views = [];

        foreach ($this->shiftRepository->findAllInRange($query->from, $query->to) as $shift) {
        ...
        return new ListShiftsByDateResponse($views);
    }
